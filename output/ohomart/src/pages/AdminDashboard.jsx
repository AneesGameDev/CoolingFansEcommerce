...
export default function AdminDashboard() {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      logger.error("loadAll failed — redirecting to login:", e?.message || e);
  ...
  const openReviewForm = (review = null) => {
    if (review) {
      ...
      if (review.created_at) {
        ...
        } catch (e) { logger.warn("Date parse error:", e?.message); }
      }
  ...
  const loadAdminUsers = useCallback(async () => {
    try {
      ...
      logger.error("Failed to load admin users:", e?.message || e);
    }
  }, []);
